...
export const PlannerSidebar = ({ mobileOpen, setMobileOpen }) => {
  ...
  useEffect(() => {
    localStorage.setItem('planner-sidebar-collapsed', isCollapsed);
  }, [isCollapsed]);
  ...
  const menuItems = [
    { name: 'Dashboard', path: '/planner/dashboard', icon: FiGrid },
    { name: 'Available Vendors', path: '/planner/vendors', icon: FiUsers },
    { name: 'Direct Chat', path: '/planner/chat/list', icon: FiMessageSquare },
  ];
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        {menuItems.map((item) => (
          <NavLink
            key={item.name}
            to={item.path}
            end={item.path === '/planner/dashboard'}
            className={({ isActive }) =>
